...
import { Shield, Image as ImageIcon, ZoomIn, Printer, X } from 'lucide-react';
import { ARTWORKS, ARTIST_INFO } from '../constants';
import { Certificate } from './Certificate';

export const DigitalCompanion: React.FC<{ artworkId: string, onClose: () => void }> = ({ artworkId, onClose }) => {
            ...
            <div className="space-y-8 flex-1">
                ...
                <div className="bg-slate-50 p-6 rounded-lg border border-slate-200">
                    ...
                    <div className="space-y-4">
                        <div className="flex justify-between text-xs border-b border-slate-200 pb-2 border-dashed"><span className="text-slate-500 font-medium">ID Registro Único</span><span className="font-mono text-slate-900 bg-white px-2 py-0.5 rounded border border-slate-200">MA-2025-{artwork.id.padStart(2,'0')}</span></div>
                        <div className="flex justify-between text-xs border-b border-slate-200 pb-2 border-dashed"><span className="text-slate-500 font-medium">Estado Legal</span><span className="text-green-700 bg-green-50 px-2 py-0.5 rounded font-bold uppercase text-[10px] tracking-wider border border-green-200">Verificado</span></div>
                        <button onClick={() => setShowCertificate(true)} className="w-full mt-2 bg-slate-900 text-white py-3 rounded text-xs font-bold uppercase tracking-wide hover:bg-gold-600 transition-all flex justify-center items-center gap-2 shadow-lg hover:shadow-gold-500/20 group"><Printer size={16} className="group-hover:scale-110"/> Ver Certificado Oficial</button>
                    </div>
                </div>
            </div>
      ...
      {showCertificate && (
        <div className="absolute inset-0 z-[110] bg-slate-900/95 backdrop-blur-md flex items-center justify-center p-4">
            <div className="bg-slate-200 p-2 md:p-8 rounded-lg shadow-2xl max-h-[95vh] overflow-y-auto w-fit relative">
                ...
                <div className="transform scale-[0.6] md:scale-90 origin-top"><Certificate artwork={artwork} /></div>
                <div className="fixed bottom-8 left-1/2 transform -translate-x-1/2"><button onClick={() => window.print()} className="bg-gold-600 text-white px-8 py-3 rounded-full flex items-center gap-2 hover:bg-gold-700 text-sm font-bold shadow-xl"><Printer size={18}/> IMPRIMIR ORIGINAL</button></div>
            </div>
        </div>
      )}
    </div>
